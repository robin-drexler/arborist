...
`)
  ...
  async [_buildDepStep] () {
    ...
    for (const edge of this[_problemEdges](node)) {
      // console.error('BIT problemEdge', [node.location, node.version], [edge.type, edge.name, edge.spec, edge.overridden, edge.error])
      if (edge.overridden)
        continue
    ...
    const placeDeps = tasks
      ...
      .map(({ edge, dep }) => new PlaceDep({
        ...
        // XXX excessive class intimacy, find a less smelly way to do this
          ...
          if (cpd.canPlaceSelf === OK) {
            ...
            if (cpd.canPlaceSelf === REPLACE) {
              ...
              for (const edgeIn of placed.edgesIn) {
                if (edgeIn === edge)
                  continue
                const { valid, overridden } = edge
                if (!valid && !overridden) {
                  // if it's already been visited, we have to re-visit
                  // otherwise, just enqueue normally.  however, we must not
                  // kj
