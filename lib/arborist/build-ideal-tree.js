  ...
  async [_buildDepStep] () {
    ...
    const placeDeps = tasks
      ...
      .map(({ edge, dep }) => new PlaceDep({
        ...
        force: this[_force],
        explicitRequests: this[_explicitRequests],
    ...
    for (const pd of placeDeps) {
      ...
      depth({
        ...
        visit: pd => {
          if (!pd.placed) {
            // didn't do anything.
            // XXX: if pd.canPlace.selfCanPlace === CONFLICT, but we placed
            // it anyway, we need to warnPeerConflict on it.
            return
          }
          const { placed, edge, canPlace: cpd } = pd
          if (cpd.canPlace === OK) {
            for (const edgeIn of placed.edgesIn) {
              if (edgeIn !== pd.edge && !edgeIn.valid && !this[_depsSeen].has(edgeIn.from)) {
                this.addTracker('idealTree', edgeIn.from.name, edgeIn.from.location)
                this[_depsQueue].push(edgeIn.from)
              }
            }
          } else if (cpd.canPlace === REPLACE) {
          ...
          } else if (cpd.canPlace === CONFLICT) {
            console.error('\nWARN PEER CONFLICT OR FAIL IT?',
              require('util').inspect(pd, { depth: Infinity }))
            console.error('')
            return
          }
