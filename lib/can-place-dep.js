...
class CanPlaceDep {
  ...
  checkCanPlace () {
    const { target, targetEdge, current } = this
  ...
  checkCanPlaceCurrent () {
    ...
    if (this.preferDedupe && !tryReplace && dep.canReplace(current)) {
      if (CONFLICT !== this.canPlacePeers(REPLACE))
        return REPLACE
    }
    ...
    for (const [entryEdge, peerSet] of entrySets.entries()) {
      ...
      if (deepestNestingTarget(entryEdge.from, current.name) !== target) {
        ...
        for (const peer of peerSet) {
          if (peer === current)
            continue
          if (deepestNestingTarget(entryEdge.from, peer.name) === target) {
            canNest = false
            break
          }
        }
    ...
    // then PlaceDep class will treat this as a KEEP.
