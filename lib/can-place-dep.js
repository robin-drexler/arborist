...
class CanPlaceDep {
  ...
  checkCanPlaceCurrent () {
    ...
    OUTER: for (const [entryEdge, peerSet] of entrySets.entries()) {
      console.error('CHECK ENTRY SET', { current: [current.location, current.version], topCurrent: this.top.current && [this.top.current.location, this.top.current.version], entryEdge, peerEntryEdge: this.peerEntryEdge, edge: this.edge })
      ...
      if (entryEdge === this.peerEntryEdge) {
        console.error('> entry edge is peer entry edge')
        continue
      }
      ...
      const nestEntry = deepestNestingTarget(entryEdge.from, entryEdge.name)
      const nestCurrent = deepestNestingTarget(nestEntry, current.name)
      if (nestCurrent !== target) {
        console.error('> deepestNestingTarget not target', entryEdge, current.name)
        // check to see if we can ALSO nest all of its peers deeper
        let canNest = true
        for (const peer of peerSet) {
          console.error('>> check peer', peer.name, peer.version)
          if (peer === current) {
            console.error('>> (is current)')
            continue
          }
          const deepestTarget = deepestNestingTarget(nestEntry, peer.name)
          if (deepestTarget === target) {
            console.error('>> cannot nest')
            canNest = false
            break
          }
        }
      ...
      if (rep && rep.satisfies(entryEdge) && ![...rep.edgesIn].some(e => e.overridden)) {
        console.error('> have replacement')
        continue
      }
      ...
      if (!rep) {
        console.error('> no replacement', entryEdge.name)
        let canReplace = true
        const entrySet = new Set([entryEdge.to])
        ENTRY_SET_LOOP: for (const entrySetNode of entrySet) {
          console.error('>> entrySetNode', [entrySetNode.location, entrySetNode.version])
          for (const edge of entrySetNode.edgesOut.values()) {
            console.error('>> check entryEdgeNode.edgesOut', edge)
            if (!edge.peer) {
              console.error('>> not a peer edge')
              entrySet.add(entrySetNode)
              continue
            }
            const rep = this.dep.parent.children.get(edge.name)
            // if no replacement, not relevant
            if (!rep) {
              console.error('>> no replacement, keep exploring peerSet')
              entrySet.add(entrySetNode)
              continue
            }
            if (rep.satisfies(edge)) {
              console.error('>> replacment satisfies')
              continue
            }
      ...
      console.error('> set conflict', entryEdge, peerSet)
      this.conflicts.set(entryEdge, peerSet)
    }

    // NB: if we're in force mode, or it's not ours and not strict, and
    // this is the only place it CAN go, then PlaceDep class will treat
    // this as a KEEP with a warning
